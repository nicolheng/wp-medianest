import { FALLBACK_MOVIE, FALLBACK_TV, FALLBACK_BOOK, FALLBACK_MUSIC } from '../services/config.js';
...
export function renderRail(containerId, items, emptyLabel, type) {
  ...
  items.forEach((item) => {
    ...
    actionContainer.setAttribute('data-action-id', idStr);
    actionContainer.setAttribute('data-action-type', type);
...
export function updateCardButtons(idStr, type, newState, itemData = null) {
  const containers = document.querySelectorAll(`[data-action-id="${idStr}"][data-action-type="${type}"]`);
  ...
  containers.forEach(container => {
    ...
    if (newState === 'history') {
      const badge = document.createElement('span');
      badge.className = 'badge bg-dark bg-opacity-75 text-light rounded-pill border border-secondary align-self-start px-3 py-2';
      let label = type === 'books' ? 'Read' : type === 'music' ? 'Listened' : 'Watched';
      badge.innerHTML = `<i class="bi bi-check2-all me-1"></i> ${label}`;
      container.append(badge);

      // If we are on the Library page, physically move the card to the History rail
      const rail = container.closest('.media-rail');
      if (rail && rail.id.startsWith('watchlist-')) {
        const cardLink = container.closest('a');
        const historyRail = document.getElementById(`history-${type}-list`);
        if (cardLink && historyRail) {
          const emptyMsg = historyRail.querySelector('p');
          if (emptyMsg) emptyMsg.remove(); // Remove the "empty" text if it's there
          historyRail.prepend(cardLink); // Move the card!
        }
      }

    } else if (newState === 'watchlist') {
      const watchBtn = document.createElement('button');
      watchBtn.className = 'btn btn-success btn-circle shadow';
      watchBtn.innerHTML = '<i class="bi bi-check-lg"></i>';
      watchBtn.title = "Mark as Done";
      ...
      removeBtn.title = "Remove";
      removeBtn.onclick = (e) => { e.preventDefault(); window.removeFromWatchlist(idStr, type); };
      container.append(watchBtn, removeBtn);
      ...
      addBtn.innerHTML = '<i class="bi bi-plus-lg"></i>';
      addBtn.title = "Add to Watchlist";
      addBtn.onclick = (e) => { e.preventDefault(); window.addToWatchlist(idStr, type, itemData); };
      container.append(addBtn);

      // If we are on the Library page, physically remove the card from the screen
      const rail = container.closest('.media-rail');
      if (rail && (rail.id.startsWith('watchlist-') || rail.id.startsWith('history-'))) {
        const cardLink = container.closest('a');
